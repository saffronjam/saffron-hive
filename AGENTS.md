...
- **Database:** SQLite. Schema migrations via golang-migrate, queries generated by sqlc from `internal/store/queries/*.sql` into `internal/store/sqlite/` (committed). `just sqlc` regenerates; `just sqlc-check` verifies the committed output matches the SQL.
