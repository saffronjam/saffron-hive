...
func (s *SQLiteStore) GetRoom(ctx context.Context, id string) (Room, error) {
	...
	var r Room
	err := row.Scan(&r.ID, &r.Name, &r.Icon, &r.CreatedAt, &r.UpdatedAt)
	if err != nil {
...
func (s *SQLiteStore) ListRooms(ctx context.Context) ([]Room, error) {
	rows, err := s.db.QueryContext(ctx, `SELECT id, name, icon, created_at, updated_at FROM rooms`)
	if err != nil {
	...
	for rows.Next() {
		var r Room
		if err := rows.Scan(&r.ID, &r.Name, &r.Icon, &r.CreatedAt, &r.UpdatedAt); err != nil {
