...
func (s *SQLiteStore) GetScene(ctx context.Context, id string) (Scene, error) {
	row := s.db.QueryRowContext(ctx,
		`SELECT id, name, created_at, updated_at FROM scenes WHERE id = ?`, id,
	)
	var sc Scene
	err := row.Scan(&sc.ID, &sc.Name, &sc.CreatedAt, &sc.UpdatedAt)
	if err != nil {
...
func (s *SQLiteStore) ListScenes(ctx context.Context) ([]Scene, error) {
	rows, err := s.db.QueryContext(ctx, `SELECT id, name, created_at, updated_at FROM scenes`)
	if err != nil {
	...
	for rows.Next() {
		var sc Scene
		if err := rows.Scan(&sc.ID, &sc.Name, &sc.CreatedAt, &sc.UpdatedAt); err != nil {
