...
>
	...
	<div class="mt-3 space-y-0.5">
		...
		{#each groupRows as row (row.group.id)}
			<div
				class="flex h-7 items-center gap-2 rounded-md px-1.5 text-sm {row.placed
		...
		{#each deviceRows as row (row.device.id)}
			<div
				class="flex items-center gap-2 rounded-md px-1.5 py-1 text-sm {row.device.disabled
					? 'opacity-60'
