	...
	>
		...
		>
			<Sheet.Header class="sr-only">
				<Sheet.Title>{m.shared_sidebar({}, locale.messageOptions())}</Sheet.Title>
				<Sheet.Description>
					{m.shared_sidebar_description({}, locale.messageOptions())}
				</Sheet.Description>
			</Sheet.Header>
		...
		<div
			...
			class={cn(
				...
				variant === "floating" || variant === "inset"
					...
					: "group-data-[collapsible=icon]:w-(--sidebar-width-icon) group-data-[side=left]:shadow-[2px_0_6px_-2px_rgb(0_0_0/var(--sidebar-shadow-opacity))] group-data-[side=right]:shadow-[-2px_0_6px_-2px_rgb(0_0_0/var(--sidebar-shadow-opacity))]",
				className
			)}
			{...restProps}
		>
