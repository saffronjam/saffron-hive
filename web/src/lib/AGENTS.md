...
- `stores/` — Svelte stores:
  ...
  - `power-intents.svelte.ts` — transient power-command feedback shared by dashboard and card controls. Keeps confirmed device state intact; holds each batch through partial reports and reconciles on confirmation, rejection, or a 10-second timeout. New commands supersede intent only for their own devices.
