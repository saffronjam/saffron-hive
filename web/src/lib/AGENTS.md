...
- `stores/` — Svelte stores:
  ...
  - `control-intents.svelte.ts` — transient power and brightness feedback shared by dashboard and card controls. Keeps confirmed device state intact; holds each batch through partial reports and reconciles on confirmation, rejection, or a fresh snapshot after a 10-second confirmation deadline. New commands supersede intent only for their own devices.
